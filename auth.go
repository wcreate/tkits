...
type CenterAuthSystem struct {
	crypto *Crypto
}

func (cas *CenterAuthSystem) GenUserToken(clientip string, uid int64, expire int, flag TokenFlag) (string, error) {
	ut := &UserToken{
		Flag:     flag,
		Uid:      uid,
		ClientIP: clientip,
...
func (cas *CenterAuthSystem) GenSysToken(clientip string, expire int) (string, error) {
	ut := &UserToken{
		Flag:     TokenSys,
		Uid:      sysUserId,
		ClientIP: clientip,
...
func (cas *CenterAuthSystem) Authenticate(clientip, token string) (*UserToken, bool) {
	ut := &UserToken{}
...
func (cas *CenterAuthSystem) Authorize(ut *UserToken, ownerUid int64, oid OperationId) bool {
	if ut != nil {
		return false
	}
...
func AuthMiddleWare() macaron.Handler {
	as := &CenterAuthSystem{crypto: scrypto}
